...
namespace PleaseThem.Core
{
  ...
  public class Pathfinder
  {
    ...
    public void InitializeSearchNodes(Map map)
    {
      ...
      for (int x = 0; x < levelWidth; x++)
      {
        for (int y = 0; y < levelHeight; y++)
        {
          ...
          var tileType = map.GetIndex(x, y);
          node.Walkable = tileType == Tiles.TileType.Grass;
    ...
    public List<Vector2> FindPath(Vector2 startPoint, Vector2 endPoint)
    {
      ...
      SearchNode startNode = searchNodes[(int)startPoint.X / Map.TileSize, (int)startPoint.Y / Map.TileSize];
      SearchNode endNode = searchNodes[(int)endPoint.X / Map.TileSize, (int)endPoint.Y / Map.TileSize];
